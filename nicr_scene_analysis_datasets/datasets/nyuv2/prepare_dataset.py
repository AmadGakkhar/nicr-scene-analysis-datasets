...
from nicr_scene_analysis_datasets.utils.img import dimshuffle, save_indexed_png
from nicr_scene_analysis_datasets.utils.io import (
    download_file,
    create_dir,
...
from nicr_scene_analysis_datasets.datasets.nyuv2 import NYUv2Meta


# https://github.com/VainF/nyuv2-python-toolkit/blob/master/splits.mat
SPLITS_FILEPATH = pkg_resources.resource_filename(__name__, "splits.mat")
# https://github.com/VainF/nyuv2-python-toolkit/blob/master/class13Mapping.mat
CLASSES_13_FILEPATH = pkg_resources.resource_filename(__name__, "class13Mapping.mat")
# https://github.com/VainF/nyuv2-python-toolkit/blob/master/classMapping40.mat
CLASSES_40_FILEPATH = pkg_resources.resource_filename(__name__, "classMapping40.mat")
MANUAL_ORIENTATIONS_TRAIN_FILEPATH = pkg_resources.resource_filename(
    __name__, "manual_orientations_train.json"
)
MANUAL_ORIENTATIONS_TEST_FILEPATH = pkg_resources.resource_filename(
    __name__, "manual_orientations_test.json"
)
...
def tar_file_to_cv2(tar, tar_filename):
    ...
    file_buffer = np.asarray(bytearray(file_extracted.read()), dtype=np.uint8)
...
def main(args=None):
    ...
    parser.add_argument(
        "--mat-filepath", default=None, help="Filepath to NYUv2 mat file."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--normal-filepath", default=None, help="Filepath to precomputd NYUv2 normals."
    )
    parser.add_argument(
    ...
    if args.enable_normal_extraction:
        ...
        if not os.path.exists(normal_filepath):
            ...
            download_file(DATASET_NORMAL_URL, normal_filepath, display_progressbar=True)
    ...
    for idxs, split in zip((train_idxs, test_idxs), ("train", "test")):
        ...
        depth_base_path = os.path.join(output_path, split_dir, NYUv2Meta.DEPTH_DIR)
        depth_raw_base_path = os.path.join(
            output_path, split_dir, NYUv2Meta.DEPTH_RAW_DIR
        )
        labels_894_base_path = os.path.join(
            output_path, split_dir, NYUv2Meta.SEMANTIC_DIR_FMT.format(894)
        )
        ...
        labels_894_colored_base_path = os.path.join(
            output_path, split_dir, NYUv2Meta.SEMANTIC_COLORED_DIR_FMT.format(894)
        )
        labels_40_colored_base_path = os.path.join(
            output_path, split_dir, NYUv2Meta.SEMANTIC_COLORED_DIR_FMT.format(40)
        )
        labels_13_colored_base_path = os.path.join(
            output_path, split_dir, NYUv2Meta.SEMANTIC_COLORED_DIR_FMT.format(13)
        )
        if args.enable_normal_extraction:
            normal_base_path = os.path.join(
                output_path, split_dir, NYUv2Meta.NORMAL_DIR
            )
        orientations_base_path = os.path.join(
            output_path, split_dir, NYUv2Meta.ORIENTATIONS_DIR
        )
        scene_class_base_path = os.path.join(
            output_path, split_dir, NYUv2Meta.SCENE_CLASS_DIR
        )
        ...
        for idx in tqdm(idxs):
            # rgb image
            cv2.imwrite(
                os.path.join(rgb_base_path, f"{idx:04d}.png"),
            ...
            cv2.imwrite(
                os.path.join(instances_base_path, f"{idx:04d}.png"), instances_write
            )

            # depth image
            cv2.imwrite(
                os.path.join(depth_base_path, f"{idx:04d}.png"), depth_images[idx]
            )

            # raw depth image
            cv2.imwrite(
                os.path.join(depth_raw_base_path, f"{idx:04d}.png"),
                raw_depth_images[idx],
            )

            # label with 1+894 classes
            label_894 = labels[idx]
            cv2.imwrite(os.path.join(labels_894_base_path, f"{idx:04d}.png"), label_894)

            # colored label image
            # (normal png16 as this type does not support indexed palettes)
            label_894_colored = colors[894][label_894]
            cv2.imwrite(
                os.path.join(labels_894_colored_base_path, f"{idx:04d}.png"),
                cv2.cvtColor(label_894_colored, cv2.COLOR_RGB2BGR),
            )

            # label with 1+40 classes
            label_40 = mapping_894_to_40[label_894].astype("uint8")
            cv2.imwrite(os.path.join(labels_40_base_path, f"{idx:04d}.png"), label_40)
            # colored label image
            # (indexed png8 with color palette)
            save_indexed_png(
                os.path.join(labels_40_colored_base_path, f"{idx:04d}.png"),
                label_40,
                colors[40],
            )
            ...
            cv2.imwrite(os.path.join(labels_13_base_path, f"{idx:04d}.png"), label_13)
            # colored label image
            # (indexed png8 with color palette)
            save_indexed_png(
                os.path.join(labels_13_colored_base_path, f"{idx:04d}.png"),
                label_13,
                colors[13],
            )
            ...
            if args.enable_normal_extraction:
                ...
                mask_img = tar_file_to_cv2(
                    normal_tar, f"normals_gt/masks/{idx:04d}.png"
                )
                ...
                normal_img = tar_file_to_cv2(
                    normal_tar, f"normals_gt/normals/{idx:04d}.png"
                )
            ...
            # scene classes
            scene_class_path = os.path.join(scene_class_base_path, f"{idx:04d}.txt")
            with open(scene_class_path, "w") as f:
                f.write(scene_types[idx])

    # splits
    np.savetxt(
        os.path.join(output_path, NYUv2Meta.SPLIT_FILELIST_FILENAMES["train"]),
        train_idxs,
        fmt="%04d",
    )
    np.savetxt(
        os.path.join(output_path, NYUv2Meta.SPLIT_FILELIST_FILENAMES["test"]),
        test_idxs,
        fmt="%04d",
    )
